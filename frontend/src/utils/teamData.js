// src/utils/teamData.js
export const teamMembers = {
  '1': {
    id: '1',
    name: 'ALICIA',
    lastName: 'VARGAS',
    position: 'Socio',
    email: 'ali_32_23@hotmail.com',
    whatsapp: '59170405747',
    image: '/images/alicia2.png',
    experience: [
      "Abogada con amplia experiencia en la gestión y defensa de procesos de asistencia familiar bajo el Proceso Extraordinario regulado en la Ley 603. Su labor abarca desde la presentación y admisión de demandas, gestión de medidas cautelares, hasta la conducción de audiencias de conciliación y emisión de sentencias. Su compromiso es proteger los derechos de los beneficiarios con prontitud, aplicando procedimientos rigurosos y eficientes que garantizan soluciones justas y oportunas. Además, maneja recursos de apelación para asegurar la correcta administración de justicia en cada caso."
    ],
    education: [
      'Licenciada en Derecho, Universidad NUR.',
      'Maestría en Derecho de Familia y Proceso Extraordinario, Universidad Pública de El Alto (UPEA).',
      'Diplomado en Medidas Cautelares y Protección Familiar, Universidad Privada Boliviana (UPB).',
      'Seminarios en Apelaciones y Recursos en Derecho Familiar, Universidad Mayor de San Andrés (UMSA).'
    ],
    description: "Especialista en derecho corporativo con amplia experiencia en asesoría legal a empresas nacionales e internacionales."
  },

  '2': {
    id: '2',
    name: 'LUCIANE',
    lastName: 'CUELLAR',
    position: 'Socia',
    email: 'mgonzalez@emba.com.bo',
    whatsapp: '59175555517',
    image: '/images/luciane2.png',
    experience: [
      "Profesional especializada en la ejecución inmediata y modificación de obligaciones alimenticias conforme a la Ley 603. La Dra. Cuellar asesorará y representará en la liquidación, notificación, intimación y ejecución judicial de pensiones alimenticias, aplicando medidas coercitivas como embargos y apremios corporales para garantizar el cumplimiento de las obligaciones. También tiene experiencia en la gestión de modificaciones y cese de asistencia familiar, acompañando a sus clientes en procedimientos que garantizan la protección y respeto de los derechos de los menores y beneficiarios."
    ],
    education: [
      'Licenciada en Derecho, Universidad NUR.',
      'Especialización en Ejecución y Modificación de Obligaciones Alimenticias, Universidad Autónoma Gabriel René Moreno (UAGRM).',
      'Diplomado en Derecho Procesal Familiar, Universidad Católica Boliviana San Pablo (UCB).',
      'Capacitación en Medidas Coercitivas y Cumplimiento de Obligaciones, Universidad Privada Franz Tamayo.'
    ],
    description: "Especialista en derecho de familia y ejecución de obligaciones alimenticias."
  },

  '3': {
    id: '3',
    name: 'NAZIRA',
    lastName: 'MORALES',
    position: 'Asociado Senior',
    email: 'moralesrojasnaz@gmail.com',
    whatsapp: '59174180816',
    image: '/images/nazira2.png',
    experience: [
      "Abogada especializada en Derecho de Familia, con amplia experiencia en procesos de divorcio y desvinculación notarial conforme a la Ley N° 603. Experta en acompañar a las partes en la gestión y resolución de conflictos familiares, garantizando procedimientos justos bajo el marco legal boliviano. Domina las modalidades de divorcio por mutuo acuerdo (vía notarial) y contencioso. Manejo integral de requisitos para desvinculación notarial, incluyendo ausencia de hijos menores, bienes gananciales y asistencia familiar. Conocimiento en procedimientos que requieren presencia física de cónyuges o representación legal."
    ],
    education: [
      'Licenciada en Derecho, Universidad NUR.',
      'Diplomado en Derecho de las Familias y del Proceso Familiar, Universidad Autónoma Gabriel René Moreno (UAGRM).',
      'Maestría en Derecho Civil y Procesal Familiar, Universidad Pública de El Alto (UPEA).',
      'Curso avanzado en Desvinculación Notarial y Divorcio, Universidad Privada Boliviana (UPB).'
    ],
    description: "Especializada en derecho administrativo y contratación estatal con más de 10 años de experiencia asesorando a entidades públicas y privadas."
  },

  '4': {
    id: '4',
    name: 'ALEJANDRA',
    lastName: 'LÓPEZ',
    position: 'Especialista Legal',
    email: 'marynelopezguardia89@gmail.com',
    whatsapp: '59176854219',
    image: '/images/lopez2.png',
    experience: [
      "La Dra. Maryne Alejandra López Guardia cuenta con amplia experiencia en procesos de divorcio bajo el Código de las Familias y Ley N° 603 de Bolivia. Su labor se enfoca en brindar asesoría y representación jurídica integral, priorizando el diálogo y la resolución pacífica de conflictos familiares. Acompaña a clientes en todas las etapas del proceso: desde demanda, contestación, excepciones, hasta audiencias y sentencia definitiva. Garantiza el respeto de derechos y obligaciones de ambas partes conforme a ley. Su compromiso es ofrecer atención ética, empática y eficiente para lograr soluciones justas que protejan los derechos de las partes."
    ],
    education: [
      'Licenciada en Derecho, Universidad NUR.',
      'Diplomado en Derecho Familiar y Niñez, Universidad Católica Boliviana San Pablo (UCB).',
      'Especialización en Gestión de Procesos de Divorcio, Universidad Mayor de San Simón (UMSS).',
      'Taller de Derecho Procesal Familiar, Universidad Privada Franz Tamayo.'
    ],
    description: "Especialista en derecho laboral con enfoque en la protección de derechos de trabajadores y asesoría integral a empresas en materia de relaciones laborales, contratación y seguridad social."
  }
};
